* 1)  creare un metodo di istanza per gestire quanto fatto da manuel
...
CLASS zcorso_cl_httpclient DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcorso_cl_httpclient IMPLEMENTATION.
...
METHOD ariba_token.
    ...
    data: lo_destination  type ref to if_http_destination,
          ...
          lo_api          type ref to zsu_cl_customizing_handler,
          lv_url          type string,
          lv_auth         type string,
          lv_cont         type string,
    ...
    if lo_api is bound.
        ...
        lv_url = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_endpoint
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = zsu_if_constants=>c_custom_dtl_ariba_token
                                           IMPORTING    ev_error         = lv_error ).

       lv_auth = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_apikey
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = CONV zrap_su_detail-configurationid( zsu_if_constants=>c_custom_dtl_ariba_api_key )
                                           IMPORTING    ev_error         = lv_error ).


       lv_accept = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_apikey
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = CONV zrap_su_detail-configurationid( zsu_if_constants=>c_custom_acceptkey )
                                           IMPORTING    ev_error         = lv_error ).

       lv_connection = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_apikey
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = CONV zrap_su_detail-configurationid( zsu_if_constants=>c_custom_connkey  )
                                           IMPORTING    ev_error         = lv_error ).

       lv_content = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_apikey
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = CONV zrap_su_detail-configurationid( zsu_if_constants=>c_custom_contentkey )
                                           IMPORTING    ev_error         = lv_error ).

       lv_encoding = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_apikey
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = CONV zrap_su_detail-configurationid( zsu_if_constants=>c_custom_encodekey )
                                           IMPORTING    ev_error         = lv_error ).

       lv_body = lo_api->get_detail_value( EXPORTING    iv_group_name    = zsu_if_constants=>c_custom_grp_apikey
                                                        iv_system        = sy-sysid
                                                        iv_customizingid = CONV zrap_su_detail-configurationid( zsu_if_constants=>c_custom_bodykey )
                                           IMPORTING    ev_error         = lv_error ).
*       ...
    endif.
