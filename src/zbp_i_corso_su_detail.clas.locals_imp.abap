CLASS LHC_SU_DETAIL DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      VAL_TRANSPORT FOR VALIDATE ON SAVE
        IMPORTING
          KEYS FOR  SU_DETAIL~val_transport .
ENDCLASS.

CLASS LHC_SU_DETAIL IMPLEMENTATION.
  ...
  rap_transport_api->validate_changes(
      ...
      table             = 'ZRAP_SU_DETAIL'
      keys              = REF #( keys )
      reported          = REF #( reported )
      failed            = REF #( failed )
      change            = REF #( change-SU_DETAIL ) ).
  ENDMETHOD.
ENDCLASS.
