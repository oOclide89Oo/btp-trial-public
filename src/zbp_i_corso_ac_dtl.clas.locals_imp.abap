CLASS LHC_AC_DTL DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      VAL_TRANSPORT FOR VALIDATE ON SAVE
        IMPORTING
          KEYS FOR  AC_DTL~val_transport .
ENDCLASS.

CLASS LHC_AC_DTL IMPLEMENTATION.
  ...
  rap_transport_api->validate_changes(
      ...
      table             = 'ZAC_RAP_DETAIL'
      keys              = REF #( keys )
      reported          = REF #( reported )
      failed            = REF #( failed )
      change            = REF #( change-AC_DTL ) ).
  ENDMETHOD.
ENDCLASS.
