CLASS LHC_AC_MASTER DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      VAL_TRANSPORT FOR VALIDATE ON SAVE
        IMPORTING
          KEYS FOR  AC_MASTER~val_transport .
ENDCLASS.

CLASS LHC_AC_MASTER IMPLEMENTATION.
  ...
  rap_transport_api->validate_changes(
      ...
      table             = 'ZRAP_AC_MASTER'
      keys              = REF #( keys )
      reported          = REF #( reported )
      failed            = REF #( failed )
      change            = REF #( change-AC_MASTER ) ).
  ENDMETHOD.
ENDCLASS.
