CLASS zcs_cl_application_job_creator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS: create_application_log_object, "creazione dell'application log
                   create_job_entry_and_catalog. "deploy del job

    METHODS: create_apj_log_object_istanza.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.
...
CLASS zcs_cl_application_job_creator IMPLEMENTATION.
...
*        out->write( |Creation of job template failed: { lx_apj_dt_content->get_text( ) }| ).
  ...
  METHOD create_apj_log_object_istanza.
