"!@testing SRVB:ZAC_SB_SINGLE_LOG
...
METHOD CREATE.
DATA:
  ls_business_data TYPE zac_db_single,
...
lo_client_proxy = CL_WEB_ODATA_CLIENT_FACTORY=>create_v2_local_proxy(
                    EXPORTING
                      is_service_key = VALUE #( service_id      = 'ZAC_SB_SINGLE_LOG'
...
lo_request = lo_client_proxy->create_resource_for_entity_set( 'ZAC_CDS_SINGLE_LOG' )->create_request_for_create( ).
...
METHOD READ_ENTITY.
DATA:
  ls_entity_key    TYPE zac_db_single,
  ls_business_data TYPE zac_db_single,
...
lo_resource = lo_client_proxy->create_resource_for_entity_set( 'ZAC_CDS_SINGLE_LOG' )->navigate_with_key( ls_entity_key ).
...
METHOD READ_LIST.
DATA:
  lt_business_data TYPE TABLE OF zac_db_single,
  lo_client_proxy  TYPE REF TO /iwbep/if_cp_client_proxy,
...
* lt_range_FIELDNAME TYPE RANGE OF zac_fname,
* lt_range_V_FIELDNAME TYPE RANGE OF sadl_gw_value_crtl_property.
...
lo_request = lo_client_proxy->create_resource_for_entity_set( 'ZAC_CDS_SINGLE_LOG' )->create_request_for_read( ).

" Create the filter tree
*lo_filter_factory = lo_request->create_filter_factory( ).
*
*lo_filter_node_1  = lo_filter_factory->create_by_range( iv_property_path     = 'FIELDNAME'
*                                                        it_range             = lt_range_FIELDNAME ).
*lo_filter_node_2  = lo_filter_factory->create_by_range( iv_property_path     = 'V_FIELDNAME'
*                                                        it_range             = lt_range_V_FIELDNAME ).
